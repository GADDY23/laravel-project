...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-2xl shadow-xl p-6 text-white">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <p class="text-sm text-indigo-100">Teacher Dashboard</p>
                <h2 class="text-3xl font-bold">Hello, {{ auth()->user()->name }}</h2>
                @if($activeTerm)
                    <p class="text-indigo-100 mt-1">Active Term: {{ $activeTerm->academic_year }} • {{ $activeTerm->semester }}</p>
                @endif
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
            ...
            <div class="flex items-center justify-between mt-2">
                <span class="text-3xl font-bold text-blue-600 dark:text-blue-400">{{ $schedules->count() }}</span>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
            ...
            <div class="flex items-center justify-between mt-2">
                <span class="text-3xl font-bold text-indigo-600 dark:text-indigo-400">{{ $schedules->unique('section_id')->count() }}</span>
                <span class="px-2 py-1 text-xs font-semibold rounded-full bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-200">Coverage</span>
            </div>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
            <p class="text-sm text-gray-500 dark:text-gray-400">Rooms Used</p>
            <div class="flex items-center justify-between mt-2">
                <span class="text-3xl font-bold text-emerald-600 dark:text-emerald-400">{{ $schedules->unique('room_id')->count() }}</span>
                <span class="px-2 py-1 text-xs font-semibold rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900 dark:text-emerald-200">Active</span>
            </div>
        </div>
    </div>
        ...
        <div class="bg-amber-50 dark:bg-amber-900/30 border-l-4 border-amber-500 rounded-xl p-4 shadow-sm">
            <div class="flex items-start">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-sm text-amber-800 dark:text-amber-200">Check notifications for updates from admin and students.</p>
                </div>
            </div>
        </div>
    @endif
            ...
            @forelse($schedules->sortBy(fn($s) => [$s->day, $s->time_start])->take(6) as $schedule)
                <div class="px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                    ...
                    <div class="text-right">
                        <p class="text-sm text-gray-700 dark:text-gray-200">{{ $schedule->section->name }}</p>
    ...
    @if($schedules->count() > 0)
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                ...
                <span class="text-sm text-gray-500 dark:text-gray-400">All sections</span>
            </div>
                                        ...
                                        <td class="relative border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/60 h-16 p-0">
                                            @if($hasSchedule && $scheduleInCell)
                                                <div class="absolute inset-0 m-1 rounded-lg border {{ $colorClasses[$colorIndex] }} shadow-sm flex flex-col justify-center px-2 text-[11px] leading-tight">
                                                    ...
                                                    <span class="text-gray-700 dark:text-gray-200">{{ $scheduleInCell['schedule']->section->name }}</span>
    ...
    @else
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-12 text-center border border-gray-200 dark:border-gray-700">
            <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">No schedules assigned yet</h3>
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Your schedule will appear here once it's been assigned.</p>
        </div>
    @endif
</div>
