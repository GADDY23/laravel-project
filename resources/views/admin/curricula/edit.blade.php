...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Curriculum</h1>
</div>

<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
    <form action="{{ route('admin.curricula.update', $curriculum) }}" method="POST" id="curriculum-form">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">School Year Start</label>
                <input type="number" name="school_year_start" id="school_year_start" value="{{ old('school_year_start', $curriculum->school_year_start ?? now()->year) }}" min="2000" max="2100" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
            ...
            <div>
                ...
                <select name="course_type" id="course_type" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    ...
                    <option value="shs" {{ old('course_type', $curriculum->course_type ?? 'college') === 'shs' ? 'selected' : '' }}>SHS</option>
                    <option value="college" {{ old('course_type', $curriculum->course_type ?? 'college') === 'college' ? 'selected' : '' }}>College</option>
                </select>
            ...
            <div>
                ...
                <select name="curriculum_type" id="curriculum_type" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    <option value="semestral" {{ old('curriculum_type', $curriculum->curriculum_type ?? 'semestral') === 'semestral' ? 'selected' : '' }}>Semestral</option>
                    <option value="trimestral" {{ old('curriculum_type', $curriculum->curriculum_type ?? 'semestral') === 'trimestral' ? 'selected' : '' }}>Trimestral</option>
                </select>
        ...
        <div class="mb-4 p-3 rounded border border-blue-200 bg-blue-50 text-sm text-blue-800">
            Generated Curriculum Code: <span class="font-semibold" id="generated-code">{{ $curriculum->curriculum_code }}</span>
        </div>
...
@php
    if (old('row_subject_ids')) {
        $oldRowsData = collect(old('row_subject_ids'))->values()->map(function ($_, $i) {
            return [
                'subject_id' => old('row_subject_ids')[$i] ?? null,
                'year_level' => old('row_year_levels')[$i] ?? '1st_year',
                'period_number' => (int) (old('row_period_numbers')[$i] ?? 1),
                'period_label' => old('row_period_labels')[$i] ?? '1st Sem',
                'prerequisite' => old('row_prerequisites')[$i] ?? '',
            ];
        })->values();
    } else {
        $oldRowsData = $existingRows->values();
    }
@endphp

<script>
const strandsByType = @json($courseStrands);
const oldRows = @json($oldRowsData);
const previousStrand = @json(old('course_strand', $curriculum->course_strand));
const years = [
