    ...
    <div class="min-h-screen md:flex">
        <aside class="w-full md:w-72 md:min-h-screen bg-gradient-to-b from-[#041b42] to-[#072d66] text-blue-100 flex flex-col">
            ...
            <nav class="p-4 space-y-2 flex-1">
                ...
                <a href="{{ route('teacher.timetable') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-blue-800/60 text-white font-medium">
        ...
        <main class="flex-1 p-4 md:p-8">
            <div class="bg-gradient-to-r from-[#0a2a5f] via-[#0d3478] to-[#0e3f90] text-white rounded-2xl p-6 shadow-lg">
                <p class="text-blue-200 text-sm">Teacher: {{ auth()->user()->name }}</p>
                <h2 class="text-2xl md:text-3xl font-bold mt-1">My Timetable</h2>
                <p class="text-blue-200 mt-2">Only your assigned schedules are shown here.</p>
            </div>

            <div class="bg-white rounded-2xl p-4 md:p-6 border border-slate-200 shadow-sm mt-6">
                @if($schedules->isEmpty())
                    <div class="text-sm text-slate-500">No assigned schedules yet for the active term.</div>
                @else
                    @php
                        $timeSlots = [];
                    ...
                    <div id="timetable-grid" class="timetable-shell timetable-view-only overflow-x-auto">
                        <div class="timetable-panels-grid">
                            <div class="timetable-panel">
                                <h3 class="text-lg font-semibold text-gray-900 mb-2">Teacher: {{ $currentTeacher->name }}</h3>
                                                ...
                                                <tr class="relative">
                                                    ...
                                                    @foreach($days as $dayIndex => $day)
                                                        @php
                                                            $hasSchedule = false;
                                                            $scheduleInCell = null;
                                                            $positions = $schedulePositionsByTeacher[$currentTeacher->id] ?? [];
